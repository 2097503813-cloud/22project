...
> 若你在**开发机**上操作，只有 §2 / §8 的**路径与端口**不同，功能描述完全一致——
> 差异对照见 [10.6 节](#106-实验室主机与开发机的差异对照)。
>
> **本文特点**：全文截图均来自**真实运行的系统**，接口返回值是**真实调用结果**；
> 关键代码片段均标注了**真实文件路径与行号**，可对照源码核查。
...
> ⚠️ 本章数据**取自实验室交付主机的实际部署**。括号中标注了开发机的对应值（如有差异）。
...
> 而实验室主机历史上是按 **5000** 部署的（服务名 `ModelManageService` + 端口 5000 是配套的）。
> 不写 `--port` 会起在 8080，与既有服务和文档地址对不上。
> 端口差异的来龙去脉见 [10.6 节](#106-实验室主机与开发机的差异对照)。
...
> 💡 这个脚本的源文件在仓库的 `testRestfulProject\tools\reset-login-accounts.py`。
> 若实验室主机上**没有**这个脚本（早期部署的机器可能没有），
> 从更新包或仓库里拷一份过去即可，它是**独立脚本**、不依赖其他改动。
...
> ⚠️ **端口被占用**是常见原因之一，而且有个**特别隐蔽的坑**：
> **Vite 开发服务器（`npm run dev`）默认也用 8080**，
> 而 `serve.py` 的默认端口也是 8080 —— 两者会撞。
> 若你曾经手动 `serve.py`（不带 `--port`）启动过，就会起在 8080 上，
> 与实验室的 5000 服务完全无关，表现为"服务明明在跑，但 5000 打不开"。
>
> 验证方法：换一个端口起，看能不能通
> ```powershell
> venv\Scripts\python.exe serve.py --port 8099
> ```
> 8099 能通、5000 不通，就说明 5000 上有别的东西（或服务没起来）。
...
本文以**实验室交付主机**为准。若你在**开发机**上照着做，只有下面这些**路径与端口**不同，
功能与操作步骤**完全一致**：

| 项目 | **实验室交付主机**（本文基准） | 开发机（参考） |
|---|---|---|
| **项目根目录** | `D:\offline_package\04-program-code\testRestfulProject` | `D:\HUAT\22project\testRestfulProject` |
| **前端源码** | `D:\offline_package\04-program-code\frontend\22project` | `D:\HUAT\22project\frontend\22project` |
| **服务名** | `ModelManageService` | 未注册服务（手动跑） |
| **端口** | **5000** | 8080（`serve.py` 默认） |
| **MySQL 程序** | `C:\Program Files\MySQL\MySQL Server 8.0\bin\` | 随项目解压的目录 |
| **启动方式** | NSSM 服务自启 + 手动 `serve.py --port 5000` | `serve.py`（默认 8080） |

> **为什么端口不一样**：实验室主机是**早期按 5000 部署**的（服务名 `ModelManageService`
> 与端口 5000 是配套的一整套）；而 `serve.py` 的**默认端口是 8080**，
> 仓库里的 NSSM 安装脚本也写的是 8080。
>
> **本项目的取舍：以实验室现有的 5000 为准，不做改动。**
> 原因：服务已经注册好、端口已经在用、防火墙可能已放行 5000，
> 为了"和脚本默认值一致"去改端口，要同时改服务配置 + 防火墙 + 所有文档地址，
> 收益为零、风险不小。**保持现状**。
>
> ⚠️ 因此有两条**必须注意**的操作约定：
>
> 1. **手动启动一定要带 `--port 5000`**，否则会起在 8080 上，与既有服务/文档地址对不上。
> 2. **不要重新运行 `05-install-service.ps1`** —— 它按服务名 `ModelPlatform` + 端口 8080 注册，
>    会在实验室主机上多注册一个服务，与 `ModelManageService` 抢端口。
...
**原因**：早期建表或插入数据时，MySQL 连接的字符集不正确，中文被按错误的
字符集编码存进了库。**注意：这是"存进去就已经错了"**，不是显示层的问题——
所以改前端无法解决，必须修正库里的数据。

**解决**：先切到 UTF-8，再用 `utf8mb4` 字符集连接，然后 UPDATE 修正中文字段。
...
> ⚠️ **两个关键点**：
> 1. **`chcp 65001` 不能省** —— 它把控制台切到 UTF-8，否则命令里的中文
>    在传给 mysql.exe 之前就已经变成乱码了。
> 2. **`--default-character-set=utf8mb4` 不能省** —— 否则连接按默认字符集协商，
>    修完还是乱码。
>
> 这个坑**只在早期部署的机器上出现过**。后来重新初始化数据库时已用正确的字符集，
> 新部署的机器不会有这个问题。如果要修 `Models` 表的中文字段，
> 把上面的 `UPDATE` 换成对应的表与列即可。

### 10.10 PowerShell 脚本无法运行（执行策略）
...
### 10.11 更新脚本报语法错误 / 中文乱码

**现象**：运行 `.ps1` 脚本时报**语法错误**（常见的是"大括号不配对"），
或者屏幕上的中文显示为乱码。

**原因**：脚本文件被保存成了 **UTF-8 无 BOM**。而 Windows PowerShell 5.1
（也就是"右键 → 使用 PowerShell 运行"调起的那个版本）在读取无 BOM 的文件时，
**会按系统 ANSI 代码页（中文系统上是 GBK）解析**。
于是中文注释被解成乱码字节，其中可能正好包含引号或大括号，
把后面的语法结构**撑坏了** —— 所以报的是"语法错误"，但根因是编码。

**这解释了为什么"脚本内容看着完全正常，却报语法错误"**：
用记事本或 VS Code 打开是正常的（它们按 UTF-8 解），
只有 PowerShell 5.1 会解错。

**解决**：

- **已修复**：后续更新包里的 `.ps1` 已统一转为 **UTF-8 with BOM**，
  并且 `make-update-package.ps1` 会确保打进包里的每个 `.ps1` 都带 BOM。
- **临时绕过**：不跑脚本，手动复制文件更新（但要注意别漏掉 `sql/` 目录，
  见 §10.7）。
- **自查方法**：用 VS Code 打开脚本，右下角应显示 `UTF-8 with BOM`；
  若只显示 `UTF-8`，需要用"以编码保存"改成带 BOM。
...
| **v3.0** | **2026-09-20** | **改为以实验室交付主机为准**（路径 `D:\offline_package\04-program-code\`、端口 **5000**、服务名 `ModelManageService`、MySQL 在 `C:\Program Files\`）；补入部署实操踩坑（中文乱码、NSSM 权限、MySQL 开机自启、PowerShell 编码与执行策略、前端缓存）；新增 §8.7 交付自查清单、§10.6 环境差异对照、附录 D 文件结构 |
