...
export const handleMenu = (menuData: Array<any>) => {
    ...
    const dynamicRoutes = [
        {
            path: '/home', name: 'home',
            // 原本指向模板自带的 /system/home/index（一张静态首页图）。
            // 这里改成平台自己的首页，避免后端菜单里再出现一个 /home 造成"两个首页"。
            component: dynamicImport(dynamicViewsModules, '/platform/home/index'),
            meta: {
                title: '首页',
                isLink: '',
                isHide: false,
                isKeepAlive: false,   // 不缓存：每次切回首页都重新挂载
                isAffix: true,
                isIframe: false,
                roles: ['admin'],
                icon: 'ele-HomeFilled'
