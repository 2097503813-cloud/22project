...
class QuantileAD(_TrainableUnivariateDetector):
    """Detector that detects anomaly based on quantiles of historical dataset.

    This detector compares time series values with user-specified quantiles
    of historical dataset, and identifies time points as anomalous when values
    are beyond the thresholds.

    Parameters
    ----------
    low: float, optional
        Quantile of historical dataset lower which a value is regarded as anomaly.
        Must between 0 and 1. Default: None, i.e. no threshold on lower side.

    high: float, optional
        Quantile of historical dataset above which a value is regarded as anomaly.
...
class InterQuartileRangeAD(_TrainableUnivariateDetector):
    ...
    historical dataset, and identifies time points as anomalous when differences
...
class GeneralizedESDTestAD(_TrainableUnivariateDetector):
    ...
    [1, 2] on historical dataset and identifies normal values vs. outliers for
