...
def plot(
    ...
    if ts is not None:
        # type check for ts
        print("isinstance(ts, pd.Series)",isinstance(ts, pd.Series))
        if isinstance(ts, pd.Series):
            print("ts.name", ts.name)
            if ts.name is None:
...
def _validate_anomaly(anomaly):
    ...
    if isinstance(anomaly, (list, pd.Series)):
        print("_validate_anomaly pass", anomaly)
        pass
